  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        if (!affiliate) {
          toast.error("Esta conta não possui perfil de afiliado")
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOtp({
        email,
        options: {
          emailRedirectTo: `${window.location.origin}/dashboard/affiliate`,
        },
      })
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[#156634] to-[#0d4a1f]">
          <div className="flex items-center justify-center h-full p-12">
            <div className="text-center text-white">
              <h3 className="text-4xl font-bold mb-4">Painel do Afiliado</h3>
              <p className="text-xl text-green-100">
                Gere comissões compartilhando eventos esportivos
              </p>
            </div>
          </div>
        </div>
